...
# (14-2)/2+1=7
fc1_w = weight_Variable([7 * 7 * 16, 120])
fc1_b = bias_Variable([120])
max_pool2_reshape = tf.reshape(max_pool2, [-1, 7 * 7 * 16])
layer3 = tf.nn.relu(tf.matmul(max_pool2_reshape, fc1_w) + fc1_b)

fc2_w = weight_Variable(([120, 10]))
fc2_b = bias_Variable([10])
result = tf.nn.softmax(tf.matmul(layer3, fc2_w) + fc2_b)
