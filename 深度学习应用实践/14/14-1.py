...
with tf.device("/cpu:0"):
    ...
    #     卷积层1
    conv1 = tf.nn.conv2d(x_images, w_conv["conv1"], strides=[1, 1, 1, 1], padding="SAME")
    ...
    # (32-3)/2+1=16 ,SAME补全
    #     LRN层， 局部响应归一化层
    norml1 = tf.nn.lrn(pool1, 4, bias=1.0, alpha=0.001 / 9.0, beta=0.75)
    #     卷积层2
    conv2 = tf.nn.conv2d(norml1, w_conv["conv2"], strides=[1, 1, 1, 1], padding="SAME")
    conv2 = tf.nn.bias_add(conv2, b_conv["conv2"])
    conv2 = tf.nn.relu(conv2)
    print(conv2)
    #     LRN层， 局部响应归一化层
    norml2 = tf.nn.lrn(conv2, 4, bias=1.0, alpha=0.001 / 9.0, beta=0.75)
    # 池化层
    pool2 = tf.nn.avg_pool(norml2, ksize=[1, 3, 3, 1], strides=[1, 2, 2, 1], padding="SAME")
...
with tf.Session() as sess:
    ...
    for i in range(200):
        for batch in range(total_batch):
            ...
            # print(type(batch_x[0]), type(batch_y))
    ...
    plt.xlabel('Iter')
    plt.ylabel('Cost')
    plt.title("lr = %f, ti=%d, acc=%f" % (learning_rate, training_iters, batch_size, test_acc))
    plt.tight_layout()
